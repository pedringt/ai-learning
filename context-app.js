(() => {
  ...
  async function saveInformation(){
    ...
    showDialog(`<span class="eyebrow">Processing</span><h2 id="dialogTitle">Analyzing update...</h2><p>Sending to AI...</p>`);
    try {
      ...
      state.reviewBannerDismissed=false; closeDialog(); updateNav(); 
      showDialog(`<span class="eyebrow">Analyzed</span><h2>Update processed by AI</h2><p><strong>${result.interpretation?.outcome==='review_recommended'?'Review recommended':'No review needed'}</strong></p><p>Topics: ${(result.interpretation?.topics||[]).join(', ')||'None'}</p><div class="dialog-actions"><button class="btn primary" data-action="go-review">Go to Review</button><button class="btn secondary" data-action="close-dialog">Done</button></div>`);
    } catch (error) {
      console.error('Error:', error);
      closeDialog();
      showDialog(`<span class="eyebrow">Error</span><h2>Could not process</h2><p>${error.message}</p><div class="dialog-actions"><button class="btn secondary" data-action="close-dialog">Close</button></div>`);
    }
  }

...
- **The historical "pass" files (`context-design-pass.js`, `context-feedback-pass.js`/`-2`/`-3`/`-4`, `context-attention-alignment.js`, `context-final-mobile.js`) were investigated 2026-09-12, not consolidated.** Despite the names, all 7 are fully live: each injects its own `<style id="...">` block into `<head>` on first run and keeps re-applying DOM patches (icon injection, class toggling, mobile ask-drawer sync) via its own `MutationObserver` watching the whole document — nothing here is dead code. The working hypothesis going in was that later files' heavy `!important` overrides would make much of the earlier files' CSS payload dead weight; a mechanical cross-file cascade analysis (identical-selector-text matching, same method as `context-tool.css`, extended across all 9 CSS sources — the stylesheet, the inline mobile-nav block, and all 7 files' injected `<style>` blocks, in their real DOMContentLoaded-listener-registration load order) found that hypothesis mostly wrong: only 100 of 5,169 declarations (2%) were actually shadowed, meaning these files are largely additive across different selectors, not layered full overrides. That 2% was still real and safe to remove by the same proof used elsewhere in this codebase, plus 39 more declarations in `context-tool.css` itself that turned out to be shadowed by these later-loading files. Verified two ways: the mechanical winner-map comparison (zero differences across all sources), and the repo's own `tools/screenshot_matrix.py` visual regression harness (42 screenshots × before/after, byte-identical). That harness needed a small fix first — its State-view navigation selectors were scoped to `.sidebar-nav`, which is hidden at the 390px phone width since the 2026-09-08 mobile-nav consolidation, so the harness had been silently broken at that width; changed to target `[data-view=...]:visible` so it resolves whichever nav (sidebar or mobile) is actually shown. **Not attempted:** consolidating the JS DOM-patch behavior itself. Whether a later pass's logic (e.g. `context-final-mobile.js`'s mobile ask-drawer sync) makes an earlier pass's equivalent obsolete needs behavioral tracing across 7 interacting `MutationObserver`s, not text analysis — a materially harder and riskier problem than anything resolved in this pass, and not something to take on speculatively.

  **Attempted twice since, both times concluded not currently safe.** The one seemingly clean case found by static reading — `context-design-pass.js`'s `nav()` sets each sidebar icon's *content* to a plain-glyph map, but `context-feedback-pass.js`'s `fixNav()` always runs right after and unconditionally overwrites that same content with its own SVG set, and can create the icon element itself if missing — looked provably dead by tracing the code alone. Removing just that piece (keeping `nav()`'s still-needed label-restructuring) broke the sidebar layout (label rendered above the icon instead of beside it) on 20 of 42 `screenshot_matrix.py` screenshots, confirmed both times with real Playwright page loads. The second attempt used live DOM inspection specifically to find the mechanism rather than reason about it statically — and still couldn't: isolated checks of the DOM after the change showed the *identical* final structure (icon span, then label span, correct order, `display:grid` computing to `"1"`/`"2"` for the grid columns) whether the fix was applied or not, in an interactive browser tab. Only the real Playwright-driven screenshot tool reproduced the actual break; a soft in-tab `navigate()` to the same URL and even a fresh tab load both failed to reproduce it reliably, meaning the bug depends on some timing detail of a true cold page load that isn't easy to reproduce or inspect interactively. Conclusion: this isn't a "needs more careful reading" problem, it's a genuine race condition across independent `MutationObserver`s in different files that resists exactly the debugging techniques normally available (step through live state, diff the DOM) because the failure doesn't reproduce outside a full fresh-navigation Playwright run. Not safely doable without a fundamentally different, much slower methodology (e.g. adding temporary instrumentation *inside* the pass files themselves to log mutation order during an actual Playwright run) — not attempted, since that changes the very files under test. Any future attempt should budget for that kind of instrumentation from the start rather than expecting static reading or interactive DOM checks to catch what only shows up under a real cold load.

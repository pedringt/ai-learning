...
- A history record with accepted_as_adjusted=true carries an ai_proposed_statement field: what State's AI originally proposed before a human revised the wording. new_statement is the only authoritative current value, exactly like any other history record -- ai_proposed_statement is provenance, never an alternate current fact, and must never be presented as what Current State says now. Use it only to answer a question specifically about what changed or what State originally proposed/suggested (e.g. "was this adjusted?", "what did State originally propose?", "what did the human change?") -- state plainly that a human revised the AI's wording before it was accepted, and give both the original and the approved wording. Do not volunteer this distinction unprompted for a plain "what is X now" question; new_statement alone answers that."""

(() => {
  ...
  function styles(){
    ...
    style.textContent=`
      ...
      .settings-rule-list{list-style:none;margin:14px 0 0;padding:0;display:grid;gap:8px}
      .settings-rule-list li{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding:12px 14px;border:1px solid var(--line,#e5e5ea);border-radius:10px;background:var(--soft,#fafafd)}
      .settings-rule-copy strong{display:block;margin-bottom:4px;font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:var(--muted,#666)}
      .settings-rule-form{display:flex;gap:8px;margin-top:16px;padding-top:16px;border-top:1px solid var(--line,#e5e5ea);align-items:flex-end}
  ...
  async function render(){
    ...
    const target=root();if(!target) return;const projectRules=await rules();if(!settingsNav.classList.contains('active')) return;
    target.innerHTML=`<article class="page settings-page">
      ...
      <section class="settings-section"><div class="settings-section-head"><div><h3>Connected sources</h3><p>Places State can gather project information from planning, discovery, documentation, and collaboration.</p></div></div><div class="settings-callout"><strong>Sources provide Evidence. They never change Current State directly.</strong></div><div class="source-list settings-source-grid" aria-label="Connected and planned sources">
      ...
      <section class="settings-section settings-danger"><div class="settings-section-head"><div><h3>Demo</h3><p>Restore Northstar to the seeded demo baseline.</p></div><div class="settings-actions"><button class="btn secondary" type="button" data-settings-action="reset-demo">Reset demo</button></div></div></section>
    </article>`;
  }

  document.addEventListener('submit',async event=>{const form=event.target.closest('form[data-settings-action="add-rule"]');if(!form)return;event.preventDefault();const input=form.elements.rule;const text=String(input?.value||'').trim();if(!text)return;const button=form.querySelector('button[type="submit"]');if(button)button.disabled=true;try{await api()?.createRule?.(text,'Interpretation');if(input)input.value='';await render();}catch(error){console.error('Could not add project rule.',error);if(button)button.disabled=false;}});
  document.addEventListener('click',async event=>{const control=event.target.closest('[data-settings-action]');if(!control)return;const action=control.dataset.settingsAction;if(action==='delete-rule'){control.disabled=true;try{await api()?.deleteRule?.(control.dataset.ruleId);await render();}catch(error){console.error('Could not remove project rule.',error);control.disabled=false;}}if(action==='reset-demo'){if(!window.confirm('Reset Northstar to the original demo data?'))return;control.disabled=true;try{await api()?.resetDemo?.();window.location.reload();}catch(error){console.error('Could not reset demo.',error);control.disabled=false;}}});

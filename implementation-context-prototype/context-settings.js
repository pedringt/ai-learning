(() => {
  ...
  function render(rulesState,slackState,connectNotice){
    ...
    target.innerHTML=`<article class="page settings-page">
      ...
      <section class="settings-section settings-slack" id="settings-slack"><div class="settings-section-head"><div class="settings-slack-intro"><h3 class="settings-slack-heading">${sourceIcon('slack')}<span>Slack</span></h3><p>Bring useful project conversations into State as Evidence. Slack never changes Current State directly.</p></div><span class="settings-status dev">In development</span></div><div class="settings-actions"><a class="btn secondary" href="${esc((api()?.base||'')+'/api/integrations/slack/oauth/start')}">${slack.health?.connected?'Reconnect Slack':'Connect Slack'}</a>${slack.health?.connected?'<button class="text-button" type="button" data-settings-action="disconnect-slack">Disconnect</button>':''}</div>${slackConnectNoticeMarkup(connectNotice)}<p class="settings-slack-status">${slackStatusLine(slack)}</p><div class="slack-preview" aria-label="Slack channel activity">${slackChannelsMarkup(slack)}</div></section>

(() => {
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      /* Sidebar: compact and self-contained. */
      .app-sidebar{position:sticky!important;top:0!important;align-self:start!important;height:calc(100vh - var(--topbar-height,0px))!important;min-height:0!important;padding-bottom:108px!important;overflow:hidden!important}
      ...
      .app-sidebar>.demo-help-button.state-help-card{position:absolute!important;left:18px!important;right:18px!important;bottom:18px!important;width:auto!important;max-width:none!important;margin:0!important;z-index:2!important}
      ...
      /* Workspace attention: keep the approved layout stable. */
      ...
      /* Workspace Current State summary: tighten the remaining vertical air. */
      .workspace-status-card{padding:13px 16px!important}
      ...
      .workspace-status-card .state-fact-preview li{padding-top:8px!important;padding-bottom:8px!important}
      ...
      .evidence-answer-callout{position:relative!important;margin:8px 0 10px!important;padding:10px 12px 10px 34px!important;border:1px solid #d8e7fb!important;border-radius:10px!important;background:#f3f8ff!important;color:#3d506c!important;font-size:12px!important;line-height:1.45!important}
      ...
      /* Settings: utility UI can be denser. */
      ...
      .settings-page .settings-rule-list li,.settings-page .source-row{padding-top:6px!important;padding-bottom:6px!important}
      ...
      /* Ask: compact composer; generation status stays next to the action. */
      ...
      #askStateDrawer .ask-state-drawer-form input{display:block!important;width:100%!important;height:64px!important;min-height:64px!important;max-height:64px!important;padding:0 62px 0 16px!important;box-sizing:border-box!important;border-radius:12px!important;font-size:14px!important;line-height:64px!important}
      #askStateDrawer .ask-state-drawer-form button{position:absolute!important;right:10px!important;top:50%!important;bottom:auto!important;transform:translateY(-50%)!important;width:42px!important;height:42px!important;border-radius:50%!important;padding:0!important}
      #askStateDrawer .ask-state-drawer-form button:disabled{opacity:.55!important;cursor:wait!important}
      ...
      @media(max-width:760px){
        ...
        .notes-page .note-index-status .note-status,.notes-page .note-index-status [class*="status"]{display:inline-flex!important;align-items:center!important;width:auto!important;max-width:100%!important;min-height:0!important;padding:5px 8px!important;border-radius:999px!important;font-size:9.5px!important;line-height:1.1!important;white-space:nowrap!important;text-transform:none!important;letter-spacing:0!important}
        .notes-page .note-status.is-neutral-status{padding:0!important;border:0!important;background:transparent!important;font-size:10px!important}
        ...
        #askStateDrawer .ask-state-drawer-form input{height:58px!important;min-height:58px!important;max-height:58px!important;line-height:58px!important;padding-left:13px!important;padding-right:55px!important;font-size:14px!important}
        #askStateDrawer .ask-state-drawer-form button{right:8px!important;width:40px!important;height:40px!important}
      }
  ...
  function styleEvidenceCallout(){
    document.querySelectorAll('#viewRoot p').forEach(p=>{
      if(/Evidence may answer this question/i.test(p.textContent||'')){
        p.classList.add('evidence-answer-callout');
        if(!/^New evidence/i.test(p.textContent||'')) p.textContent='New evidence may answer this question. Review it before State treats the question as resolved.';
      }
    });
  }
  ...
  function ensureAskStatus(){
    ...
    const loading=!!result?.querySelector('.ask-live-loading');
    const error=!!result?.querySelector('.ask-live-error');
    const hasAnswer=!!result?.textContent?.trim()&&!loading&&!error;
    const submit=form.querySelector('button[type="submit"]');
    drawer.classList.toggle('is-generating',loading);
    if(submit) submit.disabled=loading;
    status.className='state-ask-status';
    if(loading){status.classList.add('is-visible','is-loading');status.textContent='Finding the answer…';}
    else if(hasAnswer){status.classList.add('is-visible','is-ready');status.textContent='Answer ready';}
    else status.textContent='';
  }

  function wireQuickActionsCapture(){
    if(document.documentElement.dataset.stateQuickActionsCapture==='1') return;
    document.documentElement.dataset.stateQuickActionsCapture='1';
    document.addEventListener('click',e=>{
      const button=e.target.closest?.('.ask-quick-actions-polish button');
      if(!button) return;
      const label=(button.textContent||'').replace(/\s+/g,' ').trim();
      const prompt=button.dataset.q||QUICK_ACTIONS.get(label);
      if(prompt) button.dataset.reviewBatchPrompt=prompt;
    },true);
  }

(() => {
  ...
  function render(props){
    ...
    return `<article class="page project-page project-document"><header class="page-head project-page-head current-state-page-head" id="project-top"><div><span class="eyebrow">Maintained project understanding</span><h2>Current State</h2><p class="project-document-summary">The team’s current shared understanding of ${esc(projectName)}: what is settled enough to use, what remains constrained, and what should not be treated as decided yet.</p></div><div class="current-state-page-actions"><button class="btn secondary project-head-copy-context" data-action="open-copy-context">Copy context</button><button class="btn secondary project-settings-button" data-action="project-settings">Project settings</button></div></header><dl class="project-document-meta"><div><dt>Stage</dt><dd>${esc(orientation.stage)}</dd></div><div><dt>Outcome</dt><dd>${esc(orientation.outcome)}</dd></div></dl><div class="something-changed-cta"><div><strong>Something changed?</strong><span>Add new information and State will review whether Current State should change.</span></div><button class="btn primary" data-action="something-changed">Add Evidence</button></div><div class="project-outline">${areas.map(area=>projectOutlineSection(area,currentKnowledge(knowledge,area.id),pendingFor,history)).join('')||'<div class="empty-state"><h3>No Current State yet.</h3><p>Reviewed project understanding will appear here as a clean outline.</p></div>'}</div></article>`;
  }

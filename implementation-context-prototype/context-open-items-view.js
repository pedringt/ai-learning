(() => {
  ...
  function render(props){
    ...
    const reviewExplainer='<p class="blocking-detail"><strong>How reviews work:</strong> Some evidence suggests a change to Current State. Other evidence just needs a human check. When a change is proposed, choose whether to update Current State. Otherwise, simply mark the review complete.</p>';
    const reviewBody=reviewUnavailable?'<div class="open-items-empty unavailable-inline">Reviews could not be loaded. <button class="text-button" data-action="retry-hydration">Try again</button></div>':reviews.length?`${reviewExplainer}${reviews.map(reviewCardFor).join('')}`:'<div class="open-items-empty">Nothing needs your decision right now.</div>';

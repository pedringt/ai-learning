(() => {
  ...
  function reviewCard(r,expanded=true,accordion=false,sourceNote){
    ...
    const head=`<span class="review-row-head open-question-copy"><span class="open-item-label review">Review</span><span class="review-card-title open-question-title">${esc(r.summary)}</span>${sourceMeta?`<span class="review-source-meta open-question-meta">Evidence · ${esc(sourceMeta)}</span>`:''}</span>`;

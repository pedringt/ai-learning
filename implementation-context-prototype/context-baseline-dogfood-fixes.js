(() => {
  ...
  function showDialog(html){const overlay=document.getElementById('overlay'),body=document.getElementById('dialogBody');if(!overlay||!body)return false;body.innerHTML=html;overlay.hidden=false;document.body.classList.add('modal-open');requestAnimationFrame(()=>overlay.querySelector('[role="dialog"]')?.focus());return true}
  function closeDialog(){const overlay=document.getElementById('overlay'),body=document.getElementById('dialogBody');if(overlay)overlay.hidden=true;if(body)body.innerHTML='';document.body.classList.remove('modal-open')}
  ...
  // Re-open the review dialog directly. Clicking the banner's button does not work here:
  // the banner has no such button while Evidence is still analyzing, so the click would silently do nothing (#230).
  function reopenDraft(){const open=window.STATE_BASELINE_SETUP?.openDraft;if(open)open();else document.querySelector('[data-baseline-review-starting]')?.click()}
  async function saveNewFact(button){
    ...
    try{const response=await fetch(`${base}/api/baseline/manual`,{method:'POST',headers:{'Content-Type':'application/json','X-State-Project-Id':id},body:JSON.stringify({items:[{area_name:area,topic,statement}]})});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload?.detail?.error_details?.error_message||(typeof payload?.detail==='string'?payload.detail:null)||`API error ${response.status}`);document.querySelector('[data-baseline-new-fact-form]')?.remove();reopenDraft();scheduleRefresh(50)}catch(error){button.disabled=false;button.textContent='Add fact';setText(status,error?.message||'Could not add that fact.')}
  }
  function enhanceDraftDialog(){const dialog=document.querySelector('.baseline-draft-dialog');if(!dialog)return;const section=[...dialog.querySelectorAll('.baseline-draft-section')].find(s=>s.querySelector(':scope > h3')?.textContent.trim()==='Starting State');if(section&&!section.querySelector('[data-baseline-add-fact]')){const h3=section.querySelector(':scope > h3'),button=document.createElement('button');button.type='button';button.className='btn secondary small baseline-add-fact';button.dataset.baselineAddFact='';button.textContent='+ Add missing fact';h3?.after(button)}}
  function renameManualDialog(){const body=document.getElementById('dialogBody'),heading=body?.querySelector('h2');if(heading?.textContent.trim()==='Start from scratch'){heading.textContent='Enter Current State manually';const p=heading.nextElementSibling;if(p?.tagName==='P')p.textContent='Enter facts you already know should be part of the Starting State. Add as many as you need, then review everything before confirming Current State.'}}
  ...
  function syncWorkspaceEmptyStates(){
    ...
    if(attention&&title&&/caught up|workspace is ready/i.test(title.textContent||'')){
      const mode=baselineActive()?'baseline':'established';
      if(attention.dataset.dogfoodEmptyMode!==mode){
        ...
        if(mode==='baseline'){setText(title,'Your workspace is ready');setText(copy,'Add starting material above to establish Current State. Reviews and blocking Questions will appear here when they need attention.');if(link)link.hidden=true}
    ...
    document.querySelectorAll('.workspace-recent .workspace-section-hint').forEach(node=>{if(node.textContent.trim()==='No changes recorded yet.')setText(node,baselineActive()?'Changes will appear here after Starting State is confirmed and Current State begins changing.':'Changes will appear here as Current State is updated.')});

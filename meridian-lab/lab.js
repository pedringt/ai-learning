(function(){
  ...
  function route(){
    ...
    const allowed=['evals','knowledge','history','dashboard'];
    const name=requested==='support'?'evals':(allowed.includes(requested)?requested:'evals');
  ...
  $('#eval-example').addEventListener('click',()=>{$('#eval-freeform').value="My dashboard is showing yesterday's data. How do I refresh it?";$('#eval-freeform').focus();});
  $('#eval-freeform').addEventListener('keydown',event=>{if(event.key==='Enter'&&(event.metaKey||event.ctrlKey)){event.preventDefault();runExploratory();}});
  ...
  function runCase(id){
    ...
    const context=item.learning;const contextMarkup=context?`<section class="case-learning-context" aria-labelledby="case-context-${escape(item.id)}"><h4 id="case-context-${escape(item.id)}">Why this case exists</h4><div class="case-learning-grid"><div><strong>Why it matters</strong>${escape(context.why)}</div><div><strong>Diagnostic dimensions</strong>${context.dimensions.map(escape).join(' · ')}</div><div><strong>Expected failure layer</strong>${escape(context.failureLayer)}</div><div><strong>Source / guardrail assumption</strong>${escape(context.assumption)}</div></div></section>`:'';
    const target=$('#case-detail');target.dataset.runId=run.id;target.innerHTML=`<h3>${item.id} result</h3><p class="panel-intro">Automatic expectation check: <span class="badge ${comparison.automaticPass?'good':'risk'}">${comparison.automaticPass?'Matched':'Did not match'}</span></p>${contextMarkup}<div class="comparison"><div><span class="result-label">Expected</span><strong>${escape(comparison.expected)}</strong></div><div><span class="result-label">Actual</span><strong>${escape(comparison.actual)}</strong></div></div>${resultMarkup(result)}<div class="score-row"><button class="btn" data-score="pass" type="button">Pass after review</button><button class="btn btn-danger" data-score="fail" type="button">Fail after review</button></div>${learningReviewMarkup(run)}`;target.classList.add('active');

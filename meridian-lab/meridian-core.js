...
(function(global){
  ...
  const knowledge=[
    ...
    {id:'KB-102',title:'Account Login and Access Troubleshooting',category:'Account & Access',status:'current',keywords:['login','log in','locked','access','password','account'],content:'Verify the account email, check Caps Lock, use Forgot Password, and confirm cookies are enabled. For Enterprise SSO issues, contact the workspace administrator.'},
  ...
  const evalCases=[
    ...
    {id:'C-09',type:'classification',input:'I can\'t log in.',expected:{category:'Account & Access',minConfidence:.70},learning:{why:'Regression case: a plain substring match on "login" missed the two-word phrasing "log in", so this input previously fell through to Other / Needs Review at 35% confidence instead of Account & Access. Fixed by adding "log in" as a keyword variant.',dimensions:['Category','Confidence'],failureLayer:'Classification / taxonomy',assumption:'A bare login complaint, with no further detail, should still route to KB-102 rather than a low-confidence fallback.'}},

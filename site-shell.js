...
(()=>{
  ...
  /* Learning Guide calibration: quietly distinguish concepts that can be
     prepared in advance from judgment that becomes substantially stronger
     through real teams, customers, production systems, and constraints. */
  const markPracticeDepth=()=>{
    const learnPage=document.querySelector('[data-page="learn"]');
    if(!learnPage)return;

    const intro=learnPage.querySelector('.learning-curriculum-intro');
    if(intro&&!intro.querySelector('.practice-depth-legend')){
      const legend=document.createElement('span');
      legend.className='practice-depth-legend';
      legend.textContent=' · “Deepens in practice” = proficiency grows through real work.';
      intro.appendChild(legend);
    }

    const practiceHeavy=new Set([
      'Knowledge systems',
      'Reliability & graceful degradation',
      'Implementation planning',
      'Adoption & workflow change',
      'Product judgment & prioritization',
      'Vendor evaluation & enterprise procurement'
    ]);

    learnPage.querySelectorAll('.learning-item > b').forEach(title=>{
      const label=title.textContent.trim();
      if(!practiceHeavy.has(label)||title.querySelector('.practice-depth-marker'))return;
      const marker=document.createElement('span');
      marker.className='practice-depth-marker';
      marker.textContent='deepens in practice';
      title.appendChild(marker);
    });

    if(!document.getElementById('practice-depth-style')){
      const style=document.createElement('style');
      style.id='practice-depth-style';
      style.textContent=`
        [data-page="learn"] .practice-depth-legend{
          color:color-mix(in srgb,var(--muted) 82%,transparent)!important;
          font-size:11px!important;
          font-weight:600!important;
          white-space:normal!important;
        }
        [data-page="learn"] .practice-depth-marker{
        ...
        [data-page="learn"] .practice-depth-marker::before{content:'· ';}
        @media(max-width:640px){
          [data-page="learn"] .practice-depth-legend{font-size:10.5px!important;}
          [data-page="learn"] .practice-depth-marker{font-size:9px!important;margin-left:5px!important;}
        }
      `;

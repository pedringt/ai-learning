(() => {
  ...
  const state = {
    ...
    addedSample:false, pendingCreated:false, reviewBannerDismissed:false, dialogReturnFocus:null
  };
  ...
  function renderOverview(){
    ...
    root.innerHTML = `<section class="overview pristine">
      <section class="overview-heading"><div class="overview-heading-row"><div><h2>Northstar</h2></div><button class="btn primary overview-add" data-action="add-info">+ Add information</button></div></section>
      ${reviewBanner}
  ...
  function simpleNote(n){ const target=120+((n.id.charCodeAt(2)||7)*17)%111; const preview=n.text.length>target?n.text.slice(0,Math.max(80,target-3)).replace(/\s+\S*$/,'')+'…':n.text; return `<article class="simple-note note-index-row"><span class="note-date">${esc(n.date)}</span><div><h3>${esc(n.title)}</h3><span class="note-source">${esc(n.source)}</span><p>${esc(preview)}</p><button class="text-button note-open" data-action="view-note" data-note-id="${n.id}">View full notes →</button></div></article>`; }

  function renderNotes(){ root.innerHTML=`<section class="page collection-page notes-page"><div class="page-head"><div><span class="eyebrow">Source archive</span><h2>Notes</h2><p>Project source material, with short previews for scanning.</p><p class="notes-disclosure">This demo uses a mix of notes adapted from my real discovery/product work and simulated project notes created to demonstrate retrieval, review, and maintained-context workflows.</p></div><button class="btn primary notes-add" data-action="add-info">+ Add information</button></div><div class="notes-toolbar"><input class="notes-search" id="notesSearch" type="search" placeholder="Search notes" aria-label="Search notes"></div><div class="note-results simple-notes" id="notesList">${state.data.notes.map(simpleNote).join('')}</div></section>`; }

  function renderQuestions(){ root.innerHTML=`<section class="page collection-page"><div class="page-head"><div><span class="eyebrow">Known unknowns</span><h2>Questions</h2><p>Things the project is deliberately keeping unresolved until evidence establishes an answer.</p><p class="questions-staleness-note">This page is the secondary "everything waiting" view. The primary path stays ambient: an answer the workspace can't establish gets tracked as a question right where you asked it, and pending evidence relevant to what you're asking surfaces inline in the answer itself — neither requires a visit here. Not yet built: automatic resurfacing or staleness flags for questions that sit open a long time without new evidence — right now, "Stop tracking" below is the only way to retire one.</p></div><button class="btn secondary" data-action="add-question">+ Add question</button></div><div class="question-cards">${openQuestions().map(q=>`<article class="question-card" data-action="open-question" data-question-id="${q.id}" tabindex="0" role="button"><div><span>${esc(q.origin)} · Open</span><h3>${esc(q.text)}</h3><button class="text-button question-answer" data-action="answer-question" data-question-id="${q.id}">Answer question →</button></div><button class="text-button muted" data-action="stop-question" data-question-id="${q.id}">Stop tracking</button></article>`).join('') || '<div class="empty">No open questions.</div>'}</div></section>`; }

  function showDemoHelp(){
    showDialog(`<span class="eyebrow">How this demo works</span><h2 id="dialogTitle">Explore the core State interaction</h2><p>State is a deterministic behavioral prototype exploring one idea: new evidence can enter a project without automatically changing what the project treats as current.</p><div class="demo-help-steps"><div><strong>Try the core flow</strong><p>Ask about the project → open Review → review a change → ask again and see how the answer changes.</p></div><div><strong>What’s simulated</strong><p>The project data and recognized answer paths are fixed so the state behavior is repeatable. There is no live AI model or production backend.</p></div></div><div class="dialog-actions demo-help-actions"><a class="text-button" href="../implementation-context.html">Read the case study →</a><button class="btn primary" data-action="close-dialog">Start exploring</button></div>`);
  }
  ...
  function showAddDialog(prefill=''){ showDialog(`<span class="eyebrow">Explicit write path</span><h2 id="dialogTitle">Add information</h2><p>New factual information is added as evidence first. It does not change current understanding until Review.</p><textarea id="addInfoText" rows="7" placeholder="Paste a note, finding, or new project information...">${esc(prefill)}</textarea><button class="sample-link" data-action="sample-info">Try sample information</button><div class="dialog-actions"><button class="btn primary" data-action="save-info">Add to Review</button><button class="btn secondary" data-action="close-dialog">Cancel</button></div>`); }
  ...
  function saveInformation(){
    ...
    if(sample){
      const n=state.data.notes.find(n=>n.id==='n-support');
      const r=state.data.reviews.find(r=>r.id==='r-access');
      if(n)n.status='pending'; if(r)r.status='pending'; state.addedSample=true;
    } else {
      const stamp=Date.now(), noteId='n-added-'+stamp, reviewId='r-info-'+stamp;
      state.data.notes.unshift({id:noteId,title:'New project information',text,source:'Added information',date:'Aug 29',topics:[],status:'pending',reviewId});
      state.data.reviews.unshift({id:reviewId,evidenceId:noteId,topics:[],status:'pending',title:'New information needs review',summary:text.length>180?text.slice(0,177).replace(/\s+\S*$/,'')+'…':text,proposed:'Determine whether this evidence should change current understanding.',unresolved:'What, if anything, should be incorporated into maintained project understanding.',current:'This information is not part of the current reviewed understanding.',evidence:text,establishes:'A new piece of project evidence has been supplied for human review.',doesNot:'It does not become established project knowledge simply because it was added.'});
    }
    state.reviewBannerDismissed=false; closeDialog(); updateNav(); showDialog(`<span class="eyebrow">Added</span><h2 id="dialogTitle">Information is waiting in Review.</h2><p>Current project understanding has not changed.</p><div class="dialog-actions"><button class="btn primary" data-action="go-review">Go to Review →</button><button class="btn secondary" data-action="close-dialog">Done</button></div>`);
  }
  ...
  document.addEventListener('click',e=>{
    ...
    else if(act==='view-note'){ const n=state.data.notes.find(x=>x.id===a.dataset.noteId); if(n) showDialog(`<span class="eyebrow">${esc(n.source)} · ${esc(n.date)}</span><h2 id="dialogTitle">${esc(n.title)}</h2><div class="full-note"><p>${esc(n.text)}</p>${n.id==='n-security'?'<h3>Discussion outline</h3><ul><li>Human-review boundary</li><li>Unacceptable failure categories</li><li>Evidence needed before reconsidering autonomy</li><li>Escalation expectations</li></ul><p>The group agreed that the pilot should not infer a safe autonomy percentage from a leadership target. Security wants evidence across agreed high-risk categories before review removal is reconsidered.</p>':''}</div><div class="dialog-actions"><button class="btn secondary" data-action="copy-note" data-note-id="${n.id}">Copy note</button><button class="btn secondary" data-action="add-info">Add follow-up information</button><button class="btn secondary" data-action="close-dialog">Close</button></div>`); }
    else if(act==='open-question'){ const q=state.data.questions.find(x=>x.id===a.dataset.questionId); if(q) showDialog(`<span class="eyebrow">Open question</span><h2 id="dialogTitle">${esc(q.text)}</h2><p>This stays unresolved until reviewed evidence establishes an answer.</p><div class="dialog-actions"><button class="btn primary" data-action="answer-question" data-question-id="${q.id}">Add what you learned</button><button class="btn secondary" data-action="close-dialog">Close</button></div>`); }
    ...
    else if(act==='stop-question'){const q=state.data.questions.find(q=>q.id===a.dataset.questionId);if(q)q.status='stopped';render();}

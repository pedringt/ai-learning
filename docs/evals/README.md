...
- **Separate observation, not caused by this change:** the model sometimes writes an internal record id into visible text (for example "k-monitoring" or "Demo-juniper-review-elevator qualifies this risk."): **7 of 12 answers with the old guidance, 3 of 12 with the new**. The streaming view scrubs ids while the finished answer shows them as written, so the two can differ. Not fixed here.

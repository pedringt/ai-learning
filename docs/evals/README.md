...
- **The automatic "unresolved recorded as a fact" flag is mostly a false alarm.** Reading the fact text (kept with `--json`): in 5 of 6 flagged statements the considered October 14 date was correctly hedged ("under consideration", "not yet decided"); one paste run recorded "may be deferred to October 14 ... pending decision" as its own fact, which is borderline (it belongs in a Question). The scorer needs hedge detection before this rate means anything.
- **New failure class to score: invented specifics.** One paste run wrote "September 30, **2024**" although the source never states a year.

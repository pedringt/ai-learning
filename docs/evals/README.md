...
- **Failures: 1 of 24 (4%)**, on the upload route for the longest source (`structured_plan`, run 0: 0 facts, empty draft). **Its cause was not recorded**: the report did not keep the failed interpretation's error code, and the server log lines were filtered from the console. The harness now reads the failure code and message from the failed record for every failed run, so the next failure can be told apart (`schema_violation`, a semantic code, or `provider_error`, which includes hitting max_tokens). One failure in 24 is too few to say whether it is a schema violation or a truncation.
...
Not concluded: whether the 4% failure rate is real (n=24), what caused it, or whether the paste/upload area difference exists.

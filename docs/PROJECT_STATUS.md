...
`main` is production, currently at commit `0261d0e` (merged via [PR #61](https://github.com/pedringt/ai-learning/pull/61)). **Slack Phase 2, self-serve Slack OAuth, a full round of Ask reliability/UX hardening, the Slack empty-channels copy fix, a site-wide portfolio content/wording pass, and a fix for a swallowed-click bug in the State prototype are all now live in production** -- this fourth promotion was explicitly authorized by Paige on 2026-09-06. `staging` and `main` now describe the same product with no pending items.

...
_Last updated: September 21, 2026 (morning), Pacific time. Written as a handoff for a fresh session._

## At a glance (Sept 21, 2026): read this first

- **Production is live and healthy; nothing is in flight.** The production code is frozen at `24e9d00` (the last code change, PR #250); `main` may be ahead of it only by docs-only commits. Render `state-api` serves `24e9d009663b`; both Vercel projects (`state`, `ai-learning`) are READY on it; the app, the portfolio and the case study return 200.
- **Shipped Sept 19-21, all verified in production:** the Baseline dialog fix, State on its own subdomain (`state.authenticignorance.site`), light-only State (#229), the Review-isolation fix (#238), Ask meeting-prep answers written in the backend's final shape and streamed live (#246; the model labels the Northstar "What should I know?" briefing `meeting_prep` in about 9 of 12 runs, a side effect Paige accepted), a cleaner internal-id scrub (#247), and the fix for the intermittent "Ask is temporarily unavailable" when an Ask is cancelled or overlaps another (#249).
- **Freeze:** Paige's stated goal (Sept 20-21) is to freeze the site soon, and nothing further is required for it. She has not defined how strict the freeze is. Treat it as "no production changes except urgent fixes, each needing her explicit authorization for `main`" (which `CLAUDE.md` already requires for every promotion), and ask her if unsure.
- **Open work, all parked on purpose:** #240 (Baseline can invent a year the source never states; Paige chose to wait until after the freeze), #241 (should the Deep QA real-model check "VP billing note reaches Review" hard-gate? it missed once in two runs), #233 (Baseline decomposition eval; first valid results are recorded), #228's leftovers (`tools/`, `QA.md`/`RELEASE.md` wording, remove the old in-portfolio app files and the root `api/state-config.js`, make the 307 redirects permanent), #136 (area-id migration deferred by design, guard shipped), #195 and #196 (Paige's cheat sheets), #142 (a learning placeholder, leave it alone). R-016 (Baseline analysis failures) is decided in DEC-009: no automatic retry for now.
- **Verify before relying on any of this:** `curl https://state-api-6waw.onrender.com/health` (build should be `24e9d009663b`), `git log origin/main`, `gh pr list`, and the Vercel deployments of `state` and `ai-learning`.
- **Demo data baseline:** production holds only the two seeded demo projects. On Sept 21 they matched the seed exactly after a reset (Northstar: 25 Current State items, 23 Evidence, 10 History, 7 open and 12 resolved Reviews, 19 open and 1 resolved Questions; Juniper: 11, 7, 0, 6 and 1, 5 and 1). A test session run by another agent wrote to live Northstar on Sept 21 (one Evidence, one accepted Current State fact, one History entry); Paige approved `POST /api/demo/reset` with the header `X-State-Project-Id: northstar`, which restored it. If the demo drifts again, compare with the baseline and reset the same way, with her OK (risk R-019).
- **Traps that already cost time (details further down and in `RISKS.md`):** the free Vercel plan allows 100 deployments per day team-wide (skipped builds appear to count, and Tastemake shares the budget), so batch pushes and check the `Vercel – state` status (R-017); never "Promote" a staging preview deployment to production, because its `api/state-config.js` points at the staging API; production Render has no health-check path, so a deploy causes about a minute of 502s (R-018; the dashboard fix is optional and Paige chose to wait); paid model evals need Paige's OK and eval scripts (not libraries) load `.env` (#224); setting a board item to Done auto-closes its issue, so post the explanatory comment separately with `gh issue comment`; when a local harness disagrees with the deployed app, suspect the harness wiring first.
...
On the promoted code (Sept 21, `main` `24e9d00`): `make qa-fast` passes locally (703 Python tests passed, 104 skipped because they need a real model key or Postgres; all 34 `state-*-tests.js` suites green), and the whole Python suite passes with real Postgres 16.2 enabled (716 passed at the Sept 20 head; the remaining skips are real-model tests). CI (`python`, `javascript`) is green on the merged head. **State Deep QA: 14 of 14 passed, 0 skipped** on `78c5ee4` (run 35558868236; that head's app folder is identical to production's), run against the `state` Vercel project's staging URL and the staging backend. **Ask-quality eval: 8 of 8, 0 high-severity failures** (after the #246 prompt revision; the first wording scored 7 of 8). One real-model Deep QA check ("VP billing note reaches Review") missed once in two runs on Sept 21 (#241). A `make qa-fast` run must never make a real model call: after adding eval code, check that the skipped count is still 104 and grep the log for `api.anthropic.com`.
...
Deployment and demo-data risks R-017 (free-plan Vercel deployment cap), R-018 (no zero-downtime deploys on production Render) and R-019 (the live demo data is writable by anyone) are in `docs/product/RISKS.md`.
...
## Moving State to its own Vercel project and subdomain (#228): live; leftovers parked
...
| 2. Create the Vercel project `state` | **Done.** Root Directory `implementation-context-prototype`, Ignored Build Step now compares with the last deployed commit (changed Sept 20; it was `git diff HEAD^ HEAD --quiet .`, which only looked at a push's tip commit; see `implementation-context-prototype/README.md`), same protection as the portfolio project. Custom domain `state.authenticignorance.site` attached Sept 19 (step 4). |
...
| 6. Repoint `deep-qa.yml` (hard-codes the old staging URL and the portfolio Vercel project id), the Playwright/`qa/deployed` specs and `tools/`; update `QA.md` and `RELEASE.md`; trim the portfolio-only selectors that were copied into `state-shell.*` | **Partly done and live:** Deep QA runs the State app tests against the `state` project's staging URL when `VERCEL_AUTOMATION_BYPASS_SECRET_STATE` exists (it does) and passed 14 of 14 that way (Sept 21). **Still to do (parked):** `tools/`, the `QA.md`/`RELEASE.md` wording, trimming the portfolio-only selectors copied into `state-shell.*`, removing the old in-portfolio app files and the root `api/state-config.js` (staging Deep QA no longer needs them), and later making the redirects permanent (308). |
...
## Ask cancel/overlap bug (#249): fixed and live in production (PR #250, `24e9d00`)

Paige reported (Sept 20 evening Pacific) one "Ask is temporarily unavailable" while searching "billing contact" after she had pressed the X on an Ask that was still loading. **Cause** (from production logs, reproduced): the streaming Ask endpoint is a generator stepped on a pool of worker threads, and SQLite's default same-thread check made `connection.close()` raise `ProgrammingError` whenever the closing thread differed from the opening one. Cancelling always triggers it (server-side only, the client is gone); if a following request's own close lands on another thread, the code sends an `error` event after the good answer (already cached, so a retry is a cache hit). Reproduced on a real local server: the old behaviour logged the failure in 8 of 8 cancels, the fix in 0 of 8. **Fix:** `check_same_thread=False` in `db.connect_sqlite` (safe: a request uses its connection one step at a time; Postgres unaffected), with `test_db_thread_safety.py` and `test_ask_stream_cancel.py` (real socket), both failing without the fix. **Verified** on staging (real model, twice) and on production after deploy (one round: cancel an Ask mid-answer, ask "billing contact"; a normal answer, no error event, and no failure, `ProgrammingError` or traceback in the production logs for that window, which do show both requests); Deep QA 14 of 14 on the promoted head. Unknown whether the bug began with the Sept 21 backend deploy or was latent (only these two occurrences in the retained production logs).

## Ask meeting_prep streaming, fix at the source (#246, #247): live in production (PR #248, then the frontend with PR #250)

**Where production stands (Sept 21, 15:45 UTC), verified:** PR #248 (`5c5d8a8`) was merged with Paige's explicit authorization (she also accepted the meeting-prep label side effect), and PR #250 (`24e9d00`, the #249 fix) the next morning. Render production `state-api` serves `24e9d009663b` (`scripts/verify-render-deploy.sh`), clean boot logs; the `state` frontend deploy that Vercel had refused after PR #248 (the free plan's cap of 100 deployments per day, quoted as resetting at 8:20 PM Pacific) **succeeded on PR #250's merge, earlier than quoted**: the `state` production deployment is READY at `24e9d00` and the served `context-ask.js` has the live-streaming display (no `holdSections`) and the whole-slug scrub; the `ai-learning` production deployment is READY too, and the app, portfolio and case study return 200. **Lesson kept:** the free plan caps deployments at 100 per day team-wide (skipped builds appear to count, and Tastemake shares it), so batch pushes; the quoted reset time was conservative. Do NOT "Promote" a staging preview to production (its `api/state-config.js` points at the staging API). The details of the change, the measurements and the checks follow.
...
Snapshot Sept 21 (verify on the board; setting an item to Done auto-closes its issue):

- **Done and live in production:** #229, #138, #135, #139, #238, #239 (PR #237); #246 and #247 (PR #248, with the frontend deployed alongside PR #250); #249 (PR #250).
- **In progress:** #228 (the split is live: steps 1-5 done, and step 6's Deep QA repoint is live and proven; parked: `tools/`, `QA.md`/`RELEASE.md`, removing the old in-portfolio app files, permanent redirects).
- **Backlog, parked on purpose:** #240 (P2, AI behavior: Baseline can invent a year the source never states; Paige chose to wait until after the freeze), #241 (P2: the Deep QA "VP billing note reaches Review" real-model check, variance and whether it should hard-gate), #233 (P2: Baseline decomposition eval; first valid results are in `docs/evals/README.md`), #136 (P2: project-area ids; option C shipped, the migration is deferred with explicit triggers, see `docs/architecture/PROPOSAL_136_PROJECT_SCOPED_AREA_IDS.md`), #195 and #196 (cheat sheets; Paige is producing the content), #142 (**a learning placeholder that may never ship in State; leave it alone**).
...
- **Render production has no health-check path** (`state-api`, `healthCheckPath` empty), so a deploy is not zero-downtime: requests during the roughly 90-second swap get 502 (a test session hit this on Sept 21 during PR #250's deploy). Optional dashboard fix: Settings, Health Check Path `/health` (my tools cannot set it; Paige chose to wait). Render production deploys are scoped by root directory, so docs-only and frontend-only merges do not restart it (PR #235 did not).
- **The Vercel free plan caps deployments at 100 per day, team-wide** (the `state` and `ai-learning` projects and Tastemake share it; skipped builds appear to count). A refused deployment shows as the `Vercel – <project>` status "Deployment rate limited" and the API returns 402 `api-deployments-free-per-day` with a quoted reset time; on Sept 21 the cap lifted earlier than quoted, so check the real status instead of waiting. Redeploying through the Vercel API is subject to the project's Ignored Build Step, which now compares against the last deployed commit (see `implementation-context-prototype/README.md`).
- **Slack apps** (api.slack.com/apps): "State" is the staging app and "State prod" is the production app; each was checked on Sept 20 to point its Redirect URL and its Event Subscriptions request URL (Verified) at its own backend.
- **Production data safety net:** Render takes an automatic disk snapshot every 24 hours (kept at least 7 days; no manual snapshot exists, and a restore loses everything written after it). A read-only export of everything production held on Sept 20 was saved on Paige's Mac at `~/state-prod-export-2026-09-20.json` (outside the repo). Nothing on production is unique today: it is the seed data.

## Working notes for the next session (lessons from Sept 19-21)
...
- **Check the neighbouring layer before reporting a finding from one layer.** On Sept 20 I said the finished Ask answer shows raw internal ids because I read the frontend and missed that the backend scrubs first; measuring with the real scrubber showed 0 to 1 of 12 answers affected (#247). Measure with the real function.
- **A conditional approval is a decision.** "I like your recommendation" (measure first, act only if the number is high) was the decision (DEC-009); report the condition's outcome, do not keep listing it as open.
- **Paid model calls:** state the call count and the estimated cost, dry-run first, and get a yes. Reusable eval code goes in a library module with no `load_local_env()`; the runnable script stays thin and is the only thing that loads `.env`; add the script to `ENTRYPOINTS` and the library to the import-time guard in `test_eval_env_isolation.py`. A test that imports a script un-skips the live tests and makes `make qa-fast` call the model (it happened twice on Sept 20; the tell was the skipped count moving from 104 to 103).
- **A prompt change needs a before/after measurement on the affected suites.** For #246 the shape eval looked good while the Ask-quality eval caught a regression the shape eval could not see (fixed section titles leaking into a plain fact answer); the fix scoped the guidance to the `meeting_prep` job and both suites were re-measured. Do not loosen a scorer to make it pass.
- **Reproduce concurrency bugs with both threads alive.** A finished thread's id can be reused by the next one and hide a same-thread SQLite error (#249).
- **Other agents and sessions may test on the live site and write to the demo data.** Compare with the baseline above and reset Northstar the same way, with Paige's OK.
- **Deploys and 502s:** a "Failed to fetch" or 502 right after a merge is probably the deploy window, not a crash: check the Render deploy list first.

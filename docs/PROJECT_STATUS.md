...
- **In progress:** #228 (the split; steps 1-5 done and live on production, except the Slack app URL check, which only Paige can do; step 6 not started: repoint `deep-qa.yml`, `qa/deployed`, `tools/`, `QA.md`/`RELEASE.md` at the `state` project (needs a Vercel automation-bypass secret for it as a GitHub Actions secret, created by Paige), then remove the old in-portfolio app files and root `api/state-config.js`; later switch the redirects to permanent).
- **Staging** (fixed and verified on staging, awaiting a promotion PR that needs Paige's explicit authorization): #232 (`e88d9c4`: Evidence added before the project opens no longer goes to the seed project; the switcher's project id is published only once the server confirms it), #231 (`a0f4432`: a retry button in the Baseline banner and review dialog when a source's analysis fails), #227 (`7f7d09e`: Ask's prose backstop no longer rewrites "approved" when a sentence quotes a governing Current State item; **its real-model confirmation, the paid Ask-quality eval `python -m eval.run_quality_evals --suite ask`, has not been run and needs Paige's OK to spend**). Each has a comment on its issue with the root cause, tests and verification. Deep QA was 14 of 14 on each of the three heads.
- **Ready:** #136 (P2, project-area ids; downgraded from P1), #135, #138, #139 (tech-debt audits, now with Work Type Tech debt and Product Area Platform; no Priority set; each explicitly allows deciding the current state is acceptable).
- **Backlog:** #233 (P2: Baseline decomposition eval), #229 (P2: the app renders half-dark in dark-mode browsers, pre-existing), #195 and #196 (cheat sheets; Paige is producing the content and will say when the PDFs are ready), #142 (**a learning placeholder that may never ship in State; leave it alone**).
...
3. Check the relevant GitHub Issue/Project item. Start with #228 (the split): its comments hold the evidence and next steps. Steps 1-5 are live; step 6 comes next (needs a bypass secret from Paige).

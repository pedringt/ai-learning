...
Paige asked to tackle tech debt in a fresh chat right after PR #64 promoted. Progress so far, on branch `tech-debt/css-consolidation` (not yet merged to `staging`):

- **CSS override layering in `context-tool.css` — partially done 2026-09-06.** The ~30 scattered `@media` blocks (some breakpoints redefined five-plus separate times) are now 13: one consolidated block per breakpoint, plus four single-rule exceptions pinned in their original spot (each commented) because merging them would have changed which declaration wins at that breakpoint. This was verified mechanically, not eyeballed: a script modeled the CSS cascade for every selector/property across every combination of viewport width, dark mode, and `prefers-reduced-motion`, confirmed the consolidated file resolves identically to the original for all of them, and separately found 96 whole rules that were already permanently dead (shadowed by a later declaration in every case) and deleted those outright. JS behavior suites (81+18+5+1) and Python suite (328) still green; manually spot-checked in a real browser (static server + mobile viewport + light/dark) with no new console errors. **Still open:** the same layering pattern in this file's ~1,100 non-media lines (no breakpoint to mechanically partition on, so verifying a merge there is harder), and the version-stamped inline `<style>` blocks in `index.html` (untouched this pass). See README's "Known debt" for the up-to-date summary, and this branch's git history for the verification approach if it needs re-running.
- `context-app.js` is a single ~1,840-line module; splitting it was investigated and rejected for a documented reason (ES modules are CORS-blocked over `file://`, and the prototype must open from the filesystem) — re-read that reasoning in the README before reopening the idea, it's not a naive oversight. Not touched this pass.

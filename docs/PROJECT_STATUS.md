...
- **Ask relevance hardening**: a manual QA failure showed that a specific lookup such as "Who is the billing contact?" could pull semantically adjacent billing-scope records and build a confident but irrelevant answer instead of admitting that State did not know the contact. Ask now filters out zero/weak-relevance adjacent records for specific lookups and treats "unknown" as the correct outcome when the project record does not actually establish the requested fact.
- **Ask follow-up mode hardening**: follow-ups are now distinguished as fresh questions/topic shifts, dependent follow-ups, or transformations. Fresh questions no longer reinsert the previous answer above the new answer; dependent follow-ups can still use prior context; transformations intentionally reshape the prior answer. This fixes the observed behavior where a new answer started streaming and then appeared to fall back to the original answer.
- **Dependent follow-up normalization**: natural follow-ups such as `Why?`, `How do you know?`, `Where did you get that?`, `Who else?`, and similar short referential prompts keep prior-answer context. Backend and frontend normalization now agree on punctuation, fixing a CI-caught mismatch where `Why?` and `Why` could classify differently.
